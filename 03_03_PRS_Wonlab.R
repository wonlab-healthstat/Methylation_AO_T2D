### /data4/jyjo/Tutorial/PRS/uniPRS-code.rmd

library(dplyr)  ;   library("R.utils")
# PT and CT
...
uniPRS.fun = function(name.idir,name.ss,name.test,name.odir,method){
  ...
  if(method=="LDpred"){
    ...
    if(!file.exists(paste0(name.idir,"LDpred-tmp/",name.ss,".sumstats.gz"))){
      system(paste("cp",paste0(name.idir,name.ss,".sumstats"),paste0(name.idir,"LDpred-tmp/",name.ss,".sumstats")))
    ...
    #ss$beta_se <- with(ss, ifelse(beta_se == 0, beta_se+10^(-6), beta_se))
    #ss = ss[,c("CHR","BP","SNP","ALT","REF","N","P","BETA")]
    #names(ss) = c("chr","pos","rsid","a1","a0","n_eff","p","beta")
    ss1 <- ss %>% mutate(tmp_snpid = paste(chr, pos, sep = ":"))
    
    # Obtain HapMap3 SNPs & Filter out hapmap SNPs
    #info = readRDS(url("https://ndownloader.figshare.com/files/25503788"))
    info = readRDS("/home2/sjseo/Tools/HapMap3/map.rds") %>%
      mutate(tmp_snpid = paste(chr, pos, sep = ":"))
    # ss.df = ss[ss$rsid%in%info$rsid,]
    ss.df = ss1 %>% filter(tmp_snpid %in% info$tmp_snpid) %>% select(-tmp_snpid)
    ...
    info_snp = snp_match(ss.df,map.df,match.min.prop=0) # perform SNP matching
    genotype = obj.bigSNP$genotypes
    CHR = map.df$chr; POS = map.df$pos
    
    tmp_snp_asGeneticPos <-  function (infos.chr, infos.pos, dir = tempdir(), ncores = 1) 
    {
      ...
      snp_split(infos.chr, function(ind.chr, pos, dir) {
        ...
        basename <- grep(paste0("^chr", chr, "\\."), files, value = T)
    ...
    POS2 = tmp_snp_asGeneticPos(CHR,POS,
                            dir=paste0("/home2/sjseo/Tools/HapMap3"), 
                            ncores = ncore) # get the CM information from 1000 Genome
    ...
    df_beta$beta_se <- with(df_beta, ifelse(beta_se == 0, beta_se+10^(-6), beta_se))
    
    #tmp <- (df_beta$beta/(df_beta$beta_se+(10^-6)))^2
    tmp <- (df_beta$beta/df_beta$beta_se)^2
    ldsc = snp_ldsc(ld,length(ld),chi2=tmp,
    ...
    for(opt.iter in c("inf","grid","auto")){ 
      ...
      if(opt.iter=="inf"){ # infinitesimal model
        ...
        pred_prs = big_prodVec(snp_fastImputeSimple(genotype),beta_inf,
      ...
      if(opt.iter=="grid"){ # grid model : p/h2 needed 
        ...
        grid = grid.param; grid.df = data.frame(PRS=paste0("PRS",1:(20*3*2)),grid); 
        grid.df$PRS = as.character(grid.df$PRS) # designate the length of PRS depending on p&h2
        write.table(grid.df,file=paste0(name.odir,name.test,".LDpred.",opt.iter,".prs.appendix"),
                    row.names=F,quote=F)
      ...
      if(opt.iter=="auto"){ # auto model
        multi_auto = snp_ldpred2_auto(corr,df_beta,h2_init=h2_est,
                                      vec_p_init=seq_log(1e-4,0.9,length.out=max(80,ncore)),
                                      ncores=min(80,ncore))
        beta_auto = sapply(multi_auto, function(auto){auto$beta_est})
        pred_auto = big_prodMat(genotype,beta_auto,
                                ind.row = ind.test,ind.col = info_snp$`_NUM_ID_`)
...
name.idir = "/home2/sjseo/Bohun/SNParray/derived_data/03_03_PRS_Wonlab/"
name.ss = "DM_discovery"
name.test = "02_01_IDmatched"
name.odir = "/home2/sjseo/Bohun/SNParray/derived_data/03_03_PRS_Wonlab/"

fam = fread(paste0(paste0(name.idir,name.test),".fam"),data.table=F)
pheno <- read.table("/home2/sjseo/Bohun/SNParray/derived_data/IDmatch/Matched_ID.txt", header = T)
...
ggplot(data = prs_pheno, aes(x = Pheno, y = PRS, fill = Pheno)) +
  ...
  scale_y_continuous(limits=c(-5.8, 10)) +
